...
#	print "$line\n";
	...
	if ($processing_lef_layer ==0)
		...
		if ($processing_lef_layer ==1) 
			...
			if ( ($lef_layerTYPE eq "MASTERSLICE") )
				{
				$lef_derivedmaskName = "poly";
				}
			elsif ( ($lef_layerTYPE eq "CUT") && ($lef_layer_name eq "CT") )
				{
			...
			elsif ( ($lef_layerTYPE eq "IMPLANT") )
				{
				print_log( "Skipping the IMPLANT Layer...\n");
				}				
			else    { 
				print "lef_layerTYPE=$lef_layerTYPE;lef_masks=$lef_masks;lef_prev_layerTYPE=$lef_prev_layerTYPE\n";
...
print_map("LEF layer name  Milkyway Layer number\n");
